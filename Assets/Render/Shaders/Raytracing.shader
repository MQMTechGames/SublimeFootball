    ...
    SubShader {
        ...
        Pass {
            ...
            float2 Map(float3 p)
            {
            	...
            	float2 res2 = rdTube(p - float3(0., 500., 0.), 100, 200);
            	if(res2.x < res.x)
            	{
            		res = res2;
            	}
...
//					
//					// Lighting
//					float3 sunPos = float3(0., 10., 100.);
//					float3 l = normalize(sunPos - p);
//					
//					float diffuseFactor = 0.2 + 0.8 * clamp(dot(normal, l), 0., 1.);
//					color *= diffuseFactor;
//					
//					float refl = normalize(reflect(rd, normal));
//					float specFactor = pow(max(dot(refl, l), 0.), 10.) * 10.;
//					color += fixed3(1.) * specFactor;
//				}
